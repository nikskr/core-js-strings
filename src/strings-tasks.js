...
function isPalindrome(str) {
  ...
  for (let i = 0; i < Math.ceil(str.length / 2); i += 1) {
    if (
      formattedStr.charAt(i) !==
      formattedStr.charAt(formattedStr.length - 1 - i)
    ) {
      return false;
    }
  }
...
function findLongestWord(sentence) {
  const sentenceArray = sentence.replace(/[,;"'.-?!]/g, '').split(' ');
...
function reverseWords(str) {
  ...
  const result = strArray
    .map((word) => {
      ...
      for (let i = wordArray.length - 1; i > -1; i -= 1) {
...
function encodeToRot13(str) {
  ...
  const result = wordsArray.map((word) => {
    ...
    for (let i = 0; i < wordArr.length; i += 1) {
      if (wordArr[i] >= 'A' && word[i] <= 'Z') {
        if (word.charCodeAt(i) + 13 > 'Z'.charCodeAt(0)) {
          wordArr[i] = String.fromCodePoint(word.charCodeAt(i) - 13);
        } else {
          wordArr[i] = String.fromCodePoint(word.charCodeAt(i) + 13);
        }
...
function getCardId(value) {
  let multiplier;
  const lear = value.charAt(value.length - 1);
